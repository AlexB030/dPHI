...
function [pathSM,pathWtoD,pathWtoM,midwayNode,hasFailed] = generateShortestValleyfreePHITrace(listOfNodes,sourceCellC,sourceCellP,sourceCellPtoP,source,destination,helperNode)
...
[treeToHelper treeDistancesHelper] = shortestBGPtreeDestination(listOfNodes,sourceCellC,sourceCellP,sourceCellPtoP,helperNode);

%Check if the helper node can be reached. If not, no session can be
%established
pathSM=treeToHelper{source};
if(isempty(pathSM))
    hasFailed=1;
    pathSM=[];
    pathWtoD=[];
...
[treeToD treeDistancesD] = shortestBGPtreeDestination(listOfNodes,sourceCellC,sourceCellP,sourceCellPtoP,destination);
pathWtoD=treeToD{pathSM(currNode)};
%Check if the helper node can be reached. If not, no session can be
%established
if(isempty(pathWtoD))
    hasFailed=1;
...
while(foundMidway==0&&currNode>1)
    ...
    pathPrevToD=treeToD{pathSM(currNode-1)}; %shortest path from the previous node to the destination
    if(max(pathPrevToD==pathSM(currNode))==0) %Check if the current node is on the shortest path from the previous node to the destination. If this is the case then we do not backtrack
